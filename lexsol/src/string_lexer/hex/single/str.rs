...
impl StringToken {
  ...
  pub(crate) fn lex_hex<'a, S, T, Error, Container>(
    lexer: &mut SingleQuotedHexStrLexer<Lexer<'a, T>, char, HexStringError, Error>,
    ...
    Container: Default + crate::utils::Container<Error>,
  {
    ...
    lexer.bump(string_lexer.span().end);
    errs.push(HexStringError::unclosed(lexer.span().into(), LitStrDelimiterKind::Single).into());
    Err(errs)
  }
}

impl<'a, S, T, Error, Container>
  Lexable<&mut SingleQuotedHexStrLexer<Lexer<'a, T>, char, HexStringError, Error>, Container>
