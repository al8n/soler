...
{
  let span = lexer.span();
  handle_suffix::<Kind, _, _, Extras, DecimalError<_>>(lexer, Lit::lit_decimal, |l| {
  ...
  Solidity<Kind>: Lxr,
{
  ...
  match crate::handlers::str::handle_number_suffix::<_, _, DecimalError<_>>(lexer, |l| {
    DecimalError::unexpected_suffix(span.into(), l)
  }) {
    Ok(_) => Err(errs),
    Err(e) => {
  ...
  Solidity<Kind>: Lxr,
{
  ...
  match handle_suffix::<Kind, _, _, Extras, DecimalError<_>>(lexer, Lit::lit_decimal, |l| {
  ...
  Solidity<Kind>: Lxr,
{
  ...
  if remainder_str.is_empty() {
    let err = HexadecimalError::<_>::incomplete(lexer.span().into());
