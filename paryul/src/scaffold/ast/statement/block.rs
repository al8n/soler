...
use logosky::{
  ...
  error::{UnclosedBrace, UndelimitedBrace, UnexpectedEot, UnexpectedToken, UnopenedBrace},
  syntax::Language,
  utils::Span,
};
...
impl<Statement, Container, Lang> Block<Statement, Container, Lang> {
    ...
    E: ParserExtra<'a, I, Error = Error> + 'a,
  {
    ...
    .map(|db| match db {
      Ok(db) => {
        let (span, statements) = db.into_components();
        Self::new(span, statements)
      }
      Err(span) => Self::new(span, Container::default()),
    })
  }
}
