...
use logosky::utils::{IncompleteToken, Lexeme, MalformedLiteral, PositionedChar, Span, UnexpectedSuffix, human_display::DisplayHuman};
...
impl core::fmt::Display for HexadecimalLiteral {
  fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
    write!(f, "yul hex number literal")
  }
}

impl DisplayHuman for HexadecimalLiteral {
  fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
...
pub enum HexadecimalError<Char = char> {
  ...
  Incomplete(IncompleteToken<HexadecimalLiteral>),
  ...
  Malformed(MalformedLiteral<HexadecimalLiteral>),
  ...
  UnexpectedSuffix(UnexpectedSuffix<Char, HexadecimalLiteral>),
}
  ...
  fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
    match self {
      ...
      Self::UnexpectedSuffix(suffix) => match suffix.suffix() {
        ...
        Lexeme::Span(span) => {
          write!(f, "unexpected suffix after hexadecimal literal at {} after hexadecimal literal at {}", span, suffix.token())
        }
      },
      Self::Malformed(malformed) => {
        write!(f, "malformed hexadecimal literal at {}, yul hex number literal must match the regex ('0x[0-9a-fA-F]+')", malformed.span())
      }
    }
  }
}
...
impl<Char> HexadecimalError<Char> {
  /// Create an unexpected suffix error with the given token and suffix.
  #[cfg_attr(not(tarpaulin), inline(always))]
  pub const fn unexpected_suffix(
    token: Span,
    suffix: Lexeme<Char>,
  ) -> Self {
    Self::UnexpectedSuffix(UnexpectedSuffix::new(token, suffix).with_knowledge_const(HexadecimalLiteral::INIT))
  }
...
impl core::fmt::Display for DecimalLiteral {
  fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
    write!(f, "yul decimal number literal")
  }
}
...
pub enum DecimalError<Char = char> {
  ...
  Malformed(MalformedLiteral<DecimalLiteral>),
  ...
  UnexpectedSuffix(UnexpectedSuffix<Char, DecimalLiteral>),
}
...
impl<Char> DecimalError<Char> {
  ...
  pub const fn unexpected_suffix(
    ...
    Self::UnexpectedSuffix(UnexpectedSuffix::new(token, suffix).with_knowledge_const(DecimalLiteral::INIT))
  }
  ...
  fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
    match self {
      ...
      Self::UnexpectedSuffix(suffix) => match suffix.suffix() {
        ...
        Lexeme::Span(span) => {
          write!(f, "unexpected suffix after decimal literal at {} after decimal literal at {}", span, suffix.token())
        }
      },
      Self::Malformed(malformed) => {
        write!(f, "malformed decimal literal at {}, yul decimal number literal must match the regex ('0|[1-9][0-9]*')", malformed.span())
      }
    }
  }
}
