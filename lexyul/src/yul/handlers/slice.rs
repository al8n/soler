...
where
  ...
  S::Slice<'a>: AsRef<[u8]>,
{
  handle_suffix::<_, _, Extras, DecimalError<u8>>(lexer, Lit::lit_decimal, DecimalError::UnexpectedSuffix)
    .map_err(Into::into)
}
...
where
  ...
  S::Slice<'a>: AsRef<[u8]>,
{
  handle_suffix::<_, _, Extras, HexadecimalError<u8>>(lexer, Lit::lit_hexadecimal, HexadecimalError::UnexpectedSuffix)
    .map_err(Into::into)
}
